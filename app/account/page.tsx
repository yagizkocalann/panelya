...
import { ProductionAccountManager } from "./ProductionAccountManager";
...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ error?: string; saved?: string; notice?: string; verified?: string; unblocked?: string }> }) {
  ...
  const [query, actor, providerConfig] = await Promise.all([
    searchParams,
    getAccountActor(request),
    auth0WebConfig(request),
  ]);
  const providerAccount = Boolean(actor?.providerSubjectHash);
  const blockedUsers = providerAccount ? [] : await getBlockedUsers(user.id);
