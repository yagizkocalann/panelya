...
import { getAccountActor } from "../lib/account-runtime";
...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ error?: string; saved?: string; notice?: string; verified?: string; unblocked?: string }> }) {
  ...
  const [query, blockedUsers, actor, providerConfig] = await Promise.all([
    searchParams,
    getBlockedUsers(user.id),
    getAccountActor(request),
    auth0WebConfig(request),
  ]);
  const providerAccount = Boolean(actor?.providerSubjectHash);
  const canLinkProvider = Boolean(providerConfig && actor && !actor.providerSubjectHash);
  const recentAuthentication = canLinkProvider ? await hasRecentAuthentication() : false;
  return <div className="site-shell"><SiteHeader />
    <main id="main-content" className="dashboard-main wrap">
      ...
      <section className={`verification-banner${user.emailVerifiedAt ? " is-verified" : ""}`}><div><span className="pill pill--accent">{user.emailVerifiedAt ? "Doğrulandı" : "Doğrulama bekliyor"}</span><h2>E-posta güvenliği</h2><p>{user.emailVerifiedAt ? `${user.email} doğrulandı.` : providerAccount ? `${user.email} için doğrulama kimlik sağlayıcısında tamamlanmalı.` : `${user.email} için gönderilen bağlantıyı Studio yerel e-posta kutusundan aç.`}</p></div>{!user.emailVerifiedAt && !providerAccount && <form action="/api/auth/email-verification/request" method="post"><button className="button button--ghost" type="submit">Yeni bağlantı gönder</button></form>}</section>
      <div className="settings-grid">
        ...
        {providerAccount
          ? <section className="settings-card"><h2>E-posta ve şifre</h2><p>{actor?.provider === "database" ? "E-posta değişikliği ve şifre yenileme Auth0 üzerinden, taze kimlik doğrulamasıyla yürütülür." : "E-posta ve parola sosyal kimlik sağlayıcın tarafından yönetilir."}</p></section>
          : <>
        ...
        {canLinkProvider && <section className="settings-card"><h2>Production hesabına bağlan</h2><p>Aynı doğrulanmış e-postaya sahip Auth0 hesabını yalnız açık onayın ve yakın zamanda yapılmış yerel doğrulamanla bağlayabilirsin.</p>{recentAuthentication ? <form action="/api/auth/web/link" method="post"><button className="button button--primary" type="submit">Auth0 hesabını bağla</button></form> : <Link className="button button--ghost" href="/reauthenticate?return_to=/account">Önce oturumu doğrula</Link>}</section>}
        ...
        {providerAccount
          ? <section className="settings-card settings-card--danger"><h2>Hesabı sil</h2><p>Production hesap silme işlemi Auth0 ile taze doğrulama, açık etki özeti ve geri alınamaz ikinci onay gerektirir.</p></section>
