...
export default async function SessionsPage({ searchParams }: { searchParams: Promise<{ error?: string; notice?: string }> }) {
  ...
  return <div className="site-shell"><SiteHeader /><main id="main-content" className="dashboard-main wrap">
    ...
    <section className="session-list" aria-label="Aktif oturumlar">{sessions.map((session) => <article className="session-card" key={session.tokenHash}><div><span className={`pill${session.isCurrent ? " pill--accent" : ""}`}>{session.isCurrent ? "Bu cihaz" : "Aktif"}</span><h2>{deviceLabel(session.userAgent)}</h2><p>{session.userAgent?.slice(0, 130) ?? "Tarayıcı bilgisi gönderilmedi."}</p><small>Başlangıç: {new Intl.DateTimeFormat("tr-TR", { dateStyle: "medium", timeStyle: "short" }).format(new Date(session.createdAt))} · Bitiş: {new Intl.DateTimeFormat("tr-TR", { dateStyle: "medium" }).format(new Date(session.expiresAt))}</small></div>{!session.isCurrent && <form action="/api/account/sessions/revoke" method="post"><input type="hidden" name="token_hash" value={session.tokenHash} /><button className="button button--ghost" type="submit">Oturumu kapat</button></form>}</article>)}</section>
  </main><SiteFooter /></div>;
}
