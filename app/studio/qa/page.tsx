...
export default async function StudioQaPage() {
  ...
  const [publicHome, rateLimitInfo] = await Promise.all([publicSiteUrlForCurrentRequest("/"), getRateLimitInfo()]);
  const rateLimitLabel = !rateLimitInfo.available
    ? "Yapılandırma kullanılamıyor; korunan mutation’lar kapalı"
    : rateLimitInfo.edgeEnabled
  ...
  return <div className="site-shell studio-shell"><SiteHeader compact homeHref={publicHome} /><main id="main-content" className="studio-main wrap">
    ...
    <aside className="studio-notice" role={!rateLimitInfo.available ? "alert" : undefined}><strong>Kötüye kullanım koruması:</strong> {rateLimitLabel}.</aside>
