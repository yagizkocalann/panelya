...
export default async function StudioUsersPage({ searchParams }: { searchParams: Promise<{ error?: string; updated?: string; invite?: string }> }) {
  ...
  const [users, invitations, publicHome, query, recentlyAuthenticated] = await Promise.all([
    listStudioUsers(),
  ...
  return <div className="site-shell studio-shell"><SiteHeader compact homeHref={publicHome} /><main id="main-content" className="studio-main wrap">
    ...
    {!recentlyAuthenticated && <RecentAuthenticationNotice returnTo="/users" />}
    ...
    <section className="studio-section admin-invite-section" aria-labelledby="invite-title">
      ...
      <div className="admin-invite-layout">
        {recentlyAuthenticated ? <form className="stack-form admin-invite-form" action="/api/admin/invitations" method="post">
        ...
        </form> : <div className="stack-form admin-invite-form"><h3>Yeni yönetici davet et</h3><p>Davet oluşturmak için önce mevcut şifreni doğrula.</p><Link className="button button--primary" href={recentAuthenticationHref("/users")}>Şifreni doğrula</Link></div>}
        <div className="admin-invite-list" aria-label="Yönetici davetleri">
          {invitations.length ? invitations.map((invitation) => <article key={invitation.id} className={`admin-invite-card admin-invite-card--${invitation.status}`}>
            ...
            {(invitation.status === "pending" || invitation.status === "expired") && (recentlyAuthenticated ? <div className="admin-invite-actions">
            ...
            </div> : <Link className="button button--ghost" href={recentAuthenticationHref("/users")}>Davet işlemi için doğrula</Link>)}
