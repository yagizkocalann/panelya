...
- Tamamlanan: yalniz `studio.localhost` uzerinde admin ve yakin zamanda parola dogrulamasi ile acilan, kullanicinin girdigi ortak parolayi yalniz hashleyerek uc sentetik okuyucu, sekiz yayinlanmis ve iki durum testi serisi ile iliskili kutuphane/topluluk/moderasyon/outbox verisini deterministik kuran ve yalniz `qa_fixture_*`/`qa-fixture-*` ad alanini sifirlayan yerel QA veri paketi.
...
- ADR-037 / Yerel QA veri ad alani: Tekrarlanabilir kabul testi verisi production seed'ine veya mobil contracts paketine eklenmez; yalniz Studio localhost hostunda admin + same-origin + rate-limit + son 10 dakika parola dogrulamasi ile yonetilir. Manifest parola, token, gercek e-posta veya telifli icerik tasimaz. Kurulum sifreyi istekten alir ve PBKDF2 hash disinda saklamaz; sifirlama genel tablo temizligi yapmaz, yalniz `qa_fixture_*`, `qa-fixture-*` ve sentetik `@panelya.local` kayitlarini hedefler. Normal hesaplar, yonetici, bundled ozgun katalog ve rate-limit kovalari korunur; islem audit olayi uretir.
