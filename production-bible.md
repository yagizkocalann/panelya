...
- `main` hedefli pull request'ler ile `main`/`codex/web` push'lari GitHub Actions `Quality / Web quality` isinde `.nvmrc` Node surumu, `npm ci`, lint, build ve test zincirinden gecer. Branch protection kullanilamadigi surece yesil sonuc merge icin manuel zorunluluktur.
...
- ADR-024 / Platforma ayrilmis CI: Web kalite isi kok Node uygulamasini `.nvmrc` ile lint/build/test eder ve mobil SDK kurmaz. Flutter implementasyonunu `main`e tasiyan PR, sabitlenmis Flutter surumuyle `pub get`, analyze ve test calistiran ayri mobil isi birlikte getirir. Ortak sozlesme degisiklikleri iki isin fixture/contract testlerini de gecmeden tamamlanmis sayilmaz.
